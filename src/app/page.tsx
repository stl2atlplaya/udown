...
type Screen = 'landing' | 'login' | 'signup' | 'couple-setup' | 'home'
...
export default function App() {
  ...
  if (screen === 'login') return <Login onBack={() => setScreen('landing')} onSuccess={() => {}} />
    ...
    <div className={styles.landing}>
      ...
      <div className={styles.landingContent}>
        ...
        <h1 className={styles.landingH1}>
          Are you<br /><em>feeling it</em><br />tonight?
        </h1>
      ...
      <div className={styles.landingFooter}>
        Private by design. No awkward conversations required.
      </div>
    </div>
  )
}
...
function CoupleSetup({
  userId, generatedCode, setGeneratedCode, inviteCode, setInviteCode,
  coupleStatus, setCoupleStatus, onLinked
}: any) {
    ...
    <div className={styles.screen}>
      <div className={styles.screenInner}>
        ...
        <div className={styles.tabs}>
          ...
          <button className={`${styles.tab} ${tab === 'receive' ? styles.tabActive : ''}`} onClick={() => setTab('receive')}>
            I have a code
          </button>
        </div>
        ...
        {tab === 'send' && (
          <div className={styles.tabContent}>
            {!generatedCode ? (
              <>
                <p className={styles.hint}>Generate a code and send it to your partner. They enter it on their end to connect.</p>
                <button className="btn" onClick={generateCode} disabled={loading}>
                  {loading ? 'Generating...' : 'Generate invite code'}
                </button>
              </>
            ) : (
              <>
                <p className={styles.hint}>Send this code to your partner. It expires in 24 hours.</p>
                <div className={styles.codeDisplay}>{generatedCode}</div>
                <button className="btn btn-ghost" onClick={() => navigator.clipboard?.writeText(generatedCode)}>
                  Copy code
                </button>
        ...
        {tab === 'receive' && (
          <div className={styles.tabContent}>
            ...
            <div className={styles.formGroup}>
              <input
                className="input"
                placeholder="Enter code (e.g. AB3X9K)"
                value={inviteCode}
...
// ── HOME (Main daily screen) ─────────────────────────────────────────
function Home({ profile, partnerName, todayResponse, matched, onRespond, onSignOut }: {
  profile: Profile | null
  partnerName: string
  todayResponse: 'yes' | 'no' | null
  matched: boolean
  onRespond: (r: 'yes' | 'no') => Promise<void>
  onSignOut: () => void
}) {
  const [loading, setLoading] = useState(false)
  ...
  return (
    <div className={styles.screen}>
        ...
        {matched ? (
          ...
          <div className={styles.matchState}>
            ...
            <p className={styles.matchSub}>
              Tonight's the night. We'll see ourselves out.
            </p>
          ...
          <div className={styles.respondedState}>
            ...
            <h2 className={`${styles.respondedTitle} serif`}>
              {todayResponse === 'yes'
                ? 'Got it.'
                : 'No worries.'}
            </h2>
          ...
          <div className={styles.promptState}>
            ...
            <h2 className={`${styles.promptQuestion} serif`}>
              u down<br /><em>tonight?</em>
            </h2>
            <p className={styles.promptSub}>
              {partnerName} won't know what you said.<br />
              Unless you both say yes.
            </p>
            <div className={styles.responseButtons}>
              <button className="btn btn-yes" onClick={() => respond('yes')} disabled={loading}>
                Yeah 👀
              </button>
              <button className="btn btn-no" onClick={() => respond('no')} disabled={loading}>
                Not tonight
              </button>
            </div>
          </div>
      ...
      <div className={styles.homeFooter}>
        Private by design. {partnerName} can't see your answer.
      </div>
    </div>
  )
}
